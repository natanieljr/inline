 ...
 * {@link #install(Context, com.srt.appguard.monitor.multidex.MultiDexExtractor.DexLookup)}.
 * <p/>
 * <p/>
...
public final class MultiDex {
	public static final Set<String> installedApk = new HashSet<>();
	static final String TAG = "AppGuard::M::MultiDex";
	private static final String SECONDARY_FOLDER_NAME = "code_cache" + File.separator + "appguard-dexes";
	...
	public static void install(final Context context, final String additionalDex) {
		install(context, new String[]{additionalDex});
	}

	public static void install(final Context context, final String[] additionalDexFiles) {
		if (IS_VM_MULTIDEX_CAPABLE && (additionalDexFiles == null || additionalDexFiles.length == 0)) {
			Log.i(TAG, "VM has multidex support, MultiDex support library is disabled.");
			return;
		}
		if (!IS_VM_MULTIDEX_CAPABLE && Build.VERSION.SDK_INT > MAX_SUPPORTED_SDK_VERSION) {
		...
		install(context, new MultiDexExtractor.DexLookup() {
			private int i = 0;
			private int offset = 0;

			@Override
			public ZipEntry findEntry(ZipFile apk, int secondaryNumber) {
				if (additionalDexFiles != null && i < additionalDexFiles.length) {
					final ZipEntry entry = apk.getEntry(additionalDexFiles[i++]);
					if (entry != null) {
				...
				if (!IS_VM_MULTIDEX_CAPABLE) {
					final ZipEntry entry = apk.getEntry("classes" + (secondaryNumber - offset) + ".dex");
					if (entry != null) {
						Log.i(TAG, "Loading " + entry);
					}
					return entry;
				} else {
					Log.i(TAG, "VM has multidex support, ignoring classes[i].dex.");
				}
				return null;
			}
		});
	 ...
	 * MultiDexApplication for more explanation and an example.
	 *
	 * @param context application context.
	 * @throws RuntimeException if an error occurred preventing the classloader
	 *                          extension.
	 */
	private static void install(Context context, MultiDexExtractor.DexLookup gen) {
		Log.i(TAG, "install");
		...
		try {
			...
			synchronized (installedApk) {
				...
				List<File> files = MultiDexExtractor.load(context, applicationInfo, dexDir, gen, false);
				if (checkValidZipFiles(files)) {
					installSecondaryDexes(loader, dexDir, files);
				} else {
					Log.w(TAG, "Files were not valid zip files.  Forcing a reload.");
					// Try again, but this time force a reload of the zip file.
					files = MultiDexExtractor.load(context, applicationInfo, dexDir, gen, true);
					if (checkValidZipFiles(files)) {
						installSecondaryDexes(loader, dexDir, files);
					} else {
		...
		if (!files.isEmpty()) {
			if (Build.VERSION.SDK_INT >= 19) {
				try {
					...
					// Workaround for Samsung S5 (Lollipop), where there is an additional classLoader argument.
					V19SamsungS5.install(loader, files, dexDir);
				}
	...
	/**
	 * Installer for Samsung S5.
	 */
	private static final class V19SamsungS5 {
		private static void install(ClassLoader loader, List<File> additionalClassPathEntries, File optimizedDirectory)
			...
			final Field definingContextField = findField(dexPathList, "definingContext");
			final ClassLoader cl = (ClassLoader) definingContextField.get(dexPathList);
			ArrayList<IOException> suppressedExceptions = new ArrayList<>();
			expandFieldArray(dexPathList, "dexElements", makeDexElements(dexPathList, new ArrayList<>(additionalClassPathEntries), optimizedDirectory, suppressedExceptions, cl));
		 ...
		 */
		private static Object[] makeDexElements(Object dexPathList, ArrayList<File> files, File optimizedDirectory, ArrayList<IOException> suppressedExceptions, ClassLoader cl) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, NoSuchFieldException {
