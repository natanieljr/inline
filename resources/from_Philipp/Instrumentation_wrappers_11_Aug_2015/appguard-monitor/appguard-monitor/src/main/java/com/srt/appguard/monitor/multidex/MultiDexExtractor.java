...
final class MultiDexExtractor {
	 ...
	 *                     secondary dex files
	 */
	static List<File> load(Context context, ApplicationInfo applicationInfo, File dexDir, DexLookup dexLookup, boolean forceReload) throws IOException {
		...
		if (!forceReload && !isModified(context, sourceApk, currentCrc)) {
			try {
				...
				files = performExtractions(sourceApk, dexDir, dexLookup);
	...
	private static List<File> performExtractions(File sourceApk, File dexDir, DexLookup dexLookup) throws IOException {
		final String extractedFilePrefix = sourceApk.getName() + EXTRACTED_NAME_EXT;
		...
		try {
			...
			ZipEntry dexFile = dexLookup.findEntry(apk, secondaryNumber);
			while (dexFile != null) {
				String fileName = extractedFilePrefix + secondaryNumber + EXTRACTED_SUFFIX;
				File extractedFile = new File(dexDir, fileName);
				files.add(extractedFile);
				...
				secondaryNumber++;
				dexFile = dexLookup.findEntry(apk, secondaryNumber);
			}
		} finally {
			try {
				apk.close();
